  ...
  return (
    <RoleOutcomeErrorBoundary>
      ...
      >
        ...
        {/* Section 2: Business Outcome & Signal Definitions */}
        <PlaybookSection
          title="2. Business Outcome & Signal Definitions"
          badge="Product"
        >
          <EnhancedBusinessSignalsSection
            signals={signals.filter(s => s.type === 'business')}
            dependencies={dependencies}
            impacts={impacts}
          />
        </PlaybookSection>
        ...
        {/* Section 4: Process Signal Definitions */}
        <PlaybookSection
          title="4. Process Signal Definitions"
          badge="Development"
        >
        ...
        {/* Section 5: System Signal Definitions */}
        <PlaybookSection
          title="5. System Signal Definitions"
          badge="Platform SRE"
        >
...
const EnhancedBusinessSignalsSection: React.FC<{
  ...
  impacts: Impact[]
}> = ({ signals, dependencies: _dependencies, impacts: _impacts }) => {
  const businessSignals = signals.filter(s => s.type === 'business')

  // Helper to find linked dependency - removed as dependencyId no longer exists
  const findLinkedDependency = (_signal: Signal) => {
    return null // dependencyId field removed in 4-task methodology
  }
    ...
    <div>
      ...
      >
        ...
        >
          ...
          >
            {signals.filter(s => s.type === 'process').length}
          </div>
        ...
        >
          ...
          >
            {signals.filter(s => s.type === 'system').length}
          </div>
              ...
              return (
                ...
                >
                  <td style={tableCellStyle}>
                    <span
                      style={{
                        ...
                        fontWeight: '500',
                        backgroundColor:
                          signal.type === 'business' ? '#dcfce7' : '#fee2e2',
                        color:
                          signal.type === 'business' ? '#059669' : '#dc2626',
                      }}
                    >
                      {signal.type === 'business'
                        ? 'Business Signal'
                        : signal.type === 'process'
                          ? 'Process Signal'
                          : 'System Signal'}
                  ...
                  <td style={tableCellStyle}>
                    ...
                    {signal.primaryImpactType && signal.type === 'business' && (
                      <div
                        style={{
                          fontSize: '0.75rem',
                          color: '#6b7280',
...
const EnhancedProcessSignalsSection: React.FC<{
  ...
  impacts: Impact[]
  telemetryMappings: TelemetryMappingItem[]
}> = ({ signals, impacts, telemetryMappings: _telemetryMappings }) => {
...
const EnhancedSystemSignalsSection: React.FC<{
  ...
  services: any[]
  telemetryMappings: TelemetryMappingItem[]
}> = ({ signals, services, telemetryMappings: _telemetryMappings }) => {
  if (signals.length === 0) {
    return (
      <div
        style={{
