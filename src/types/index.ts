...
export interface Stakeholder {
  ...
  name: string
  role?: string
  // REMOVED: relationship field - not in new design
...
/**
 * Signal interface - BOS methodology Task 3
 * Enhanced signal definition with business logic and impact types
 * Updated for 4-task methodology
 */
export interface Signal {
  id?: string
  name: string
  type: 'business' | 'process' | 'system' // CLEAN: removed 'kpi'
  description?: string
  businessLogicDefinition?: string // NEW FIELD
  primaryImpactType?: 'financial' | 'experience' | 'legal' | 'operational' // NEW FIELD
  owner?: string
  metricName?: string
  threshold?: string
  alertConditions?: string[]
  observableUnit?: string
  // REMOVED: dependencyId - not in new design
}
