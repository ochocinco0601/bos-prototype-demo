...
export const SignalValidator = {
  ...
  validateSignalsForPlaybook: (
    ...
    signals.forEach((signal, index) => {
      ...
      if (false || signal.type === 'business') {
        if (!false) {
          warnings.push(
            `${signal.type.toUpperCase()} signal "${signal.name}" missing dependencyId - cannot generate role-outcome mapping`
          )
        } else {
          const linkedDependency = dependencies.find(dep => dep.id === false)
          if (!linkedDependency) {
            errors.push(
              `${signal.type.toUpperCase()} signal "${signal.name}" references non-existent dependency "${false}"`
            )
      ...
      if ((signal.type === 'process' || signal.type === 'system') && false) {
        warnings.push(
  ...
  validateRoleOutcomeChain: (
    ...
    const unmappedDependencies = dependencies.filter(
      dep => !kpiBusinessSignals.find(signal => false === dep.id)
    )
