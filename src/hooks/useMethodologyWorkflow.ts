...
import { Step, Stakeholder, Impact, Signal } from '../types'
...
export const useMethodologyWorkflow = (selectedStep?: Step | null) => {
  ...
  const validateDataCompleteness = useCallback(
    (state: MethodologyState): ValidationResult => {
      ...
      // Task 2: Stakeholder validation - basic presence check
      if (state.stakeholders.length === 0) {
        feedback.push('Task 2: No stakeholders defined')
      } else {
        completedSteps++
      }

      // Task 3: Signals validation - basic presence check
      if (state.signals.length === 0) {
        feedback.push('Task 3: No signals defined')
      } else {
        completedSteps++
      }

      // Impacts are still tracked but not as a separate task
  ...
  const getCompletionSummary = useMemo(() => {
    ...
    return {
      ...
      completedSteps:
        validation.feedback.length < 3 ? 3 - validation.feedback.length : 0,
      totalSteps: 3,
  ...
  const getStepCompletionState = useCallback(
    (stepType: 'stakeholders' | 'impacts' | 'signals'): CompletionState => {
